...
def exact_change_dp(x, coins):
    ...
    for target in range(smallest_coin, len(fewest_coins)):
        # Can we use any of the coins to make the target?
        options = []
        for coin in coins:
            prev = target - coin # Best count without this coin
            count = fewest_coins[prev] if prev >= 0 else None
            if not count is None:
                options.append(count + 1)
        fewest_coins[target] = min(options) if len(options) > 0 else None

    return fewest_coins[target]
